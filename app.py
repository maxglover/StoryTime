...
def index():
    if request.method == 'POST':
        ...
        entities, sentiment, dialogue = analyze_text(story_text)
        
        # Use the story text directly as the prompt for image generation
        prompt = story_text
        image_url = generate_image(prompt, OPENAI_API_KEY)
        
        # Use the first dialogue found for the speech bubble, if any
        speech_text = dialogue[0] if dialogue else "No dialogue found."
        
        # Add speech bubble to the image
        image = add_speech_bubble(image_url, speech_text)
